package de.debuglevel.evasysmiddleware.coursetype
...
class CourseTypeService(
    ...
    val count: Int
        get() {
            logger.debug { "Getting course types count..." }

            val count = this.getAll().count()

            logger.debug { "Got course types count: $count" }
            return count
        }
...
//    }
    ...
    fun getAll(): Set<CourseType> {
        logger.debug { "Getting all courses..." }

        val courseTypes = soapService.port.getCourseTypes(true).courseTypes.toSet()
            .map { course -> course.fromSoap() }
            .toSet()

        logger.debug { "Got ${courseTypes.size} courses" }
        return courseTypes
    }
